    ...
    if (auto x = std::dynamic_pointer_cast<ast::SelectStmt>(parse)) {
        ...
        for (auto &sv_sel_col : x->cols) {
            ...
            if (has_aggr && has_non_aggr && x->group == nullptr) {
                throw AmbiguousColumnError(
                    "SELECT list contains both an aggregated and a non-aggregated column without GROUP BY clause");
            }
        ...
        if (query->cols.empty()) {
            ...
            for (auto &col : all_cols) {
                TabCol sel_col = {
                    .tab_name = col.tab_name, .col_name = col.name, .alias = col.alias, .aggr = ast::NO_AGGR};
                query->cols.push_back(sel_col);
            }
        } else {
        ...
        if (x->group != nullptr) {
            ...
            for (auto &sel_col : query->cols) {
                if (sel_col.aggr == ast::NO_AGGR) {
                    ...
                    if (!found) {
                        throw AmbiguousColumnError(
                            "SELECT list contains non-aggregated column that is not in GROUP BY clause");
                    }
                }
            }
        }
        ...
        for (const auto &clause : x->set_clauses) {
            query->set_clauses.push_back({// 补全表名
...
void Analyze::check_where_clause(const std::vector<std::string> &tab_names, std::vector<Condition> &conds,
    ...
    for (auto &cond : conds) {
        // where 子句不能有聚合函数
        if ((!is_having && (cond.lhs_col.aggr != ast::NO_AGGR)) ||
            (!cond.is_rhs_val && cond.rhs_col.aggr != ast::NO_AGGR)) {
        ...
        if (cond.lhs_col.aggr == ast::AGGR_TYPE_COUNT && cond.lhs_col.col_name == "*") {
            ...
            if (cond.is_rhs_val) {
                ...
                case TYPE_FLOAT:
                    ...
                    break;
                case TYPE_STRING:
                    cond.rhs_val.init_raw(lhs_col->len > cond.rhs_val.str_val.size()
                                              ? lhs_col->len
                                              : cond.rhs_val.str_val.size()); // 多的补0
                    break;
                case TYPE_DATE:
                    cond.rhs_val.init_raw(sizeof(int));
                    break;
